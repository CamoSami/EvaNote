		...
		extends AppCompatActivity
{
    ...
    private void SetListeners()
    {
        ...
        this.binding.backButton.setOnClickListener(view ->
        {
            ...
            else
            {
                ...
                {
                    boolean isRemoved = this.todoListNotes.removeIf(
                            todoListNote -> todoListNote.getId().equals(this.todoListNote.getId()));
        ...
        this.binding.addNewTodoBtn.setOnClickListener(view ->
        {
            ...
            popupMenu.setOnMenuItemClickListener(item ->
            {
                if (item.getItemId() == R.id.newTodoList)
                {
                    ...
                    DialogAddTodoListItemBinding dialogAddTodoListItemBinding =
                            DialogAddTodoListItemBinding.inflate(getLayoutInflater());
                    builder.setView(dialogAddTodoListItemBinding.getRoot());

                    builder.setPositiveButton("Add", (dialog, which) ->
                    {
                        String newTitle =
                                dialogAddTodoListItemBinding.todoListItemTitle.getText().toString();
    ...
    {
        TodoListNote todoList = new TodoListNote(title.length() > 0 ? title : "New Todo List", false);

        TodoNote todoNote = new TodoNote("", false);
